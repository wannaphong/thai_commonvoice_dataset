# The original file is from https://github.com/ekapolc/Thai_commonvoice_split/blob/main/prepare_dataset/prepare_dataset.py
import random
import pandas as pd
import json
import os
...
validated_df_in = pd.read_csv(os.path.join(after,"validated_in.tsv"),sep="\t")

path_to_dataset = os.path.join(after,'validated.tsv')
dataset = open(path_to_dataset, 'r', encoding='utf-8')

# duration file
with open(os.path.join(after,"cv-8.0-validated.json"), encoding="utf-8") as f:
    duration_data_list = [json.loads(line) for line in f.readlines()]

train = open(os.path.join(after,'train.tsv'), 'w', encoding="utf-8")
test_uniq = open(os.path.join(after,'test_uniq.tsv'), 'w', encoding="utf-8")
dev_uniq = open(os.path.join(after,'dev_uniq.tsv'), 'w', encoding="utf-8")
test = open(os.path.join(after,'test.tsv'), 'w', encoding="utf-8")
dev = open(os.path.join(after,'dev.tsv'), 'w', encoding="utf-8")

train_before = []
with open(os.path.join(before,'train.tsv'), 'r', encoding="utf-8") as f:
    train_before=[i.strip() for i in f.readlines()]
del train_before[0]

test_uniq_before = []
with open(os.path.join(before,'test_uniq.tsv'), 'r', encoding="utf-8") as f:
    test_uniq_before=[i.strip() for i in f.readlines()]
del test_uniq_before[0]

dev_uniq_before = []
with open(os.path.join(before,'dev_uniq.tsv'), 'r', encoding="utf-8") as f:
    dev_uniq_before=[i.strip() for i in f.readlines()]
del dev_uniq_before[0]

test_before = []
with open(os.path.join(before,'test.tsv'), 'r', encoding="utf-8") as f:
    test_before=[i.strip() for i in f.readlines()]
del test_before[0]

dev_before = []
with open(os.path.join(before,'dev.tsv'), 'r', encoding="utf-8") as f:
    dev_before=[i.strip() for i in f.readlines()]
del dev_before[0]
...
test_uniq_old = []
for i in test_uniq_before:
    file = i.split()[0]
    d=validated_df_in.loc[validated_df_in['path'] == file]
    test_uniq_old.append('	'.join(d.astype(str).values.flatten().tolist()))

dev_uniq_old = []
for i in dev_uniq_before:
    file = i.split()[0]
    d=validated_df_in.loc[validated_df_in['path'] == file]
    dev_uniq_old.append('	'.join(d.astype(str).values.flatten().tolist()))

test_old = []
for i in test_before:
    file = i.split()[0]
    d=validated_df_in.loc[validated_df_in['path'] == file]
    test_old.append('	'.join(d.astype(str).values.flatten().tolist()))

dev_old = []
for i in dev_before:
    file = i.split()[0]
    d=validated_df_in.loc[validated_df_in['path'] == file]
    dev_old.append('	'.join(d.astype(str).values.flatten().tolist()))

train_old = []
for i in train_before:
    file = i.split()[0]
    d=validated_df_in.loc[validated_df_in['path'] == file]
    train_old.append('	'.join(d.astype(str).values.flatten().tolist()))

#test_uniq.write('\n'.join(test_uniq_before)+'\n')
#dev_uniq.write('\n'.join(dev_uniq_before)+'\n')
#test.write('\n'.join(test_before)+'\n')
#dev.write('\n'.join(dev_before)+'\n')
#train.write('\n'.join(train_before)+'\n')
